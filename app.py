...
def verify_all_sector_groupings(config: Config, coloring: dict, kempe_sectors: list, color_pairing: tuple):
    def verify_all_sector_groupings_recurse(grouping: Grouping, sector: int, groupings: list, groupings_size: int):
        ...
        for group in available:
            ...
            verify_all_sector_groupings_recurse(grouping, sector + 1, groupings, groupings_size)
    ...
    verify_all_sector_groupings_recurse(grouping, 1, valid_groupings, vgsize)
    ...
    for grouping in valid_groupings:
        if COLOR_SWITCHER is not do_color_switching_2:
            result = COLOR_SWITCHER(config, coloring, kempe_sectors, grouping, color_pairing)
        else:
            result = COLOR_SWITCHER(config, coloring, grouping)
        #result = do_color_switching(config, coloring, kempe_sectors, grouping, color_pairing)
        #result = do_color_switching_3(config, coloring, kempe_sectors, grouping, color_pairing)
...
def color_graphs_all(multi_thread=True):
    if multi_thread:
        Parallel(n_jobs=8)(
            delayed(special_k_to_the_ggd)(graph_arr[i], i) for i in range(len(graph_arr)))  # Color all configs
    else:
...
# for sel in [0,1,2,3]: # Do some heavy lifting before this, thermal throttling could skew results
#     COLOR_SWITCHER = do_color_switching
#     timed_reducibility_check(sel)
#     COLOR_SWITCHER = do_color_switching_2
#     timed_reducibility_check(sel)
#     COLOR_SWITCHER = do_color_switching_3
#     timed_reducibility_check(sel)
